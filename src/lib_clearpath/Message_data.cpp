...
ostream& DataPowerSystem::printMessage(ostream& stream) 
{
	...
	for(int i=0; i<num_bat; ++i) {
        ...
        switch(getDescription(0).getType()) {
            case BatteryDescription::EXTERNAL: stream << "External" << endl; break;
            case BatteryDescription::LEAD_ACID: stream << "Lead-Acid" << endl; break;
            case BatteryDescription::NIMH: stream << "NiMH" << endl; break;
            case BatteryDescription::GASOLINE: stream << "Internal Combustion Engine" << endl; break;
            default: stream << "Unknown Type" << endl; break;
        }
	}
